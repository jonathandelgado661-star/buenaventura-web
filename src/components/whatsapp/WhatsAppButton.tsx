"use client";

import { buildWhatsAppUrl } from "@/lib/whatsapp";
import { trackEvent, type AnalyticsPayload } from "@/lib/analytics";

interface WhatsAppButtonProps {
  message: string;
  children: React.ReactNode;
  className?: string;
  variant?: "primary" | "secondary" | "ghost" | "outline-light";
  size?: "sm" | "md" | "lg";
  trackPayload?: AnalyticsPayload;
  icon?: boolean;
}

const base =
  "inline-flex items-center justify-center gap-2 rounded-full font-semibold transition-all duration-300 active:scale-[0.97] whitespace-nowrap";

const variants: Record<NonNullable<WhatsAppButtonProps["variant"]>, string> = {
  primary:
    "bg-gold-500 text-forest-950 hover:bg-gold-400 shadow-lg shadow-gold-600/20 hover:shadow-xl hover:shadow-gold-600/30",
  secondary:
    "bg-forest-800 text-cream-50 hover:bg-forest-700",
  ghost: "bg-white/10 text-white backdrop-blur-sm hover:bg-white/20 border border-white/30",
  "outline-light":
    "border-2 border-cream-100 text-cream-50 hover:bg-cream-50 hover:text-forest-900",
};

const sizes: Record<NonNullable<WhatsAppButtonProps["size"]>, string> = {
  sm: "px-4 py-2 text-sm",
  md: "px-6 py-3 text-base",
  lg: "px-8 py-4 text-base md:text-lg",
};

export default function WhatsAppButton({
  message,
  children,
  className = "",
  variant = "primary",
  size = "md",
  trackPayload,
  icon = true,
}: WhatsAppButtonProps) {
  return (
    <a
      href={buildWhatsAppUrl(message)}
      target="_blank"
      rel="noopener noreferrer"
      onClick={() => trackEvent("whatsapp_click", trackPayload)}
      className={`${base} ${variants[variant]} ${sizes[size]} ${className}`}
    >
      {icon && (
        <svg viewBox="0 0 32 32" className="h-5 w-5 shrink-0 fill-current" aria-hidden="true">
          <path d="M16.001 2.667C8.64 2.667 2.667 8.64 2.667 16c0 2.457.664 4.86 1.925 6.965L2.667 29.333l6.53-1.892A13.28 13.28 0 0 0 16 29.333c7.36 0 13.333-5.973 13.333-13.333S23.36 2.667 16 2.667Zm0 24.166a10.79 10.79 0 0 1-5.505-1.507l-.395-.234-3.876 1.123 1.147-3.78-.257-.39A10.8 10.8 0 1 1 26.833 16 10.83 10.83 0 0 1 16 26.833Zm5.925-8.09c-.324-.163-1.917-.946-2.214-1.054-.297-.108-.513-.163-.729.163-.216.325-.837 1.054-1.026 1.271-.189.216-.378.244-.702.081-.324-.163-1.367-.504-2.605-1.607-.963-.858-1.613-1.918-1.802-2.243-.189-.325-.02-.5.143-.663.146-.146.324-.379.486-.568.163-.19.216-.325.324-.542.108-.216.054-.406-.027-.568-.081-.163-.729-1.756-.999-2.406-.263-.632-.53-.546-.729-.556l-.621-.011c-.216 0-.568.081-.865.406-.297.325-1.134 1.108-1.134 2.702s1.161 3.133 1.323 3.35c.163.216 2.285 3.49 5.537 4.894.774.334 1.377.534 1.848.683.776.247 1.482.212 2.041.129.623-.093 1.917-.784 2.187-1.541.27-.758.27-1.407.189-1.542-.081-.135-.297-.216-.622-.379Z" />
        </svg>
      )}
      {children}
    </a>
  );
}
